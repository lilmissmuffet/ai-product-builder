...
export default function Home() {
  ...
  function selectProject(next:Project|null) {selectedProjectRef.current=next;setProject(next)}
  function open(p:Project) {selectProject(p);setForm({name:p.name,website_url:p.website_url,product_description:p.product_description,target_customer:p.target_customer});setError("")}
  function reset() {selectProject(null);setForm(empty);setError("")}
  async function save() {
    ...
    if(result.error) { console.error("Supabase project save failed:", result.error); return setError(result.error.message); } const p=result.data as Project;selectProject(p);setProjects(x=>[p,...x.filter(y=>y.id!==p.id)]);
  }
  async function api(path:string,body:object,trace?: (message:string)=>void) {const {data:{session:activeSession}}=await supabase.auth.getSession();if(!activeSession)throw new Error("Your session expired. Please sign in again.");trace?.(`Calling ${path}.`);const r=await fetch(path,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${activeSession.access_token}`},body:JSON.stringify(body)});trace?.(`${path} returned HTTP ${r.status}.`);const raw=await r.text();console.info(`[Build My Product] Raw ${path} response:`,raw);trace?.(`Raw ${path} response: ${raw}`);let d;try {d=JSON.parse(raw);trace?.("Response JSON parsing succeeded.");} catch(parseError) {console.error("[Build My Product] Response JSON parsing failed:",parseError);trace?.("Response JSON parsing failed.");throw new Error(`Could not parse the server response: ${parseError instanceof Error?parseError.message:"Unknown parsing error"}`);}if(!r.ok)throw new Error(d.error||"Request failed.");return d}
  async function persist(p:Project) {const {data,error}=await supabase.from("projects").update({...form,analysis:p.analysis,concept:p.concept,conversation:p.conversation}).eq("id",p.id).select().single();if(error) { console.error("Supabase project persistence failed:", error); throw new Error(error.message); } const saved=data as Project;setProjects(x=>[saved,...x.filter(y=>y.id!==saved.id)]);return saved}
  async function analyze() {if(!project){await save();return}setBusy("analyze");setError("");try{const {analysis}=await api("/api/analyze",form);const p={...project,analysis};const saved=await persist(p);selectProject(saved)}catch(e){setError(e instanceof Error?e.message:"Analysis failed.")}finally{setBusy("")}}
  async function build() {setBuildDiagnostics([]);traceBuild("Build button clicked.");if(!project?.analysis) {const message="Run an analysis before building.";traceBuild(message);return setError(message);}setBusy("build");setError("");try{traceBuild("Preparing the /api/build request.");const {concept}=await api("/api/build",{...form,analysis:project.analysis},traceBuild);if(!concept)throw new Error("The build response was successful but did not include a concept.");traceBuild("Build response includes a concept.");const p={...project,concept,conversation:[{role:"assistant" as const,content:"Your initial concept is ready. Tell me what you would like to change."}]};const saved=await persist(p);traceBuild("Concept response saved to Supabase.");selectProject(saved);traceBuild("Project state updated with the saved concept.");}catch(e){const message=e instanceof Error?e.message:"Build failed.";console.error("[Build My Product] Build flow failed:",e);traceBuild(`Build failed: ${message}`);setError(message)}finally{setBusy("")}}
